...
from dataloader import get_visualnews_NE_cap
...
for special_token_type in ['domain', 'date', 'topic', 'title', 'captions', 'article',]:
...
print("specialized tokenizer length: ",len(tokenizer)) # 50272
...
train_dataset = get_visualnews_NE_cap(data_args, tokenizer=tokenizer) if training_args.do_train else None
eval_dataset = get_visualnews_NE_cap(data_args, tokenizer=tokenizer, evaluate=True) if training_args.do_eval else None
