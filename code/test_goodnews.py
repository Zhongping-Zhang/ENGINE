...
from dataloader import Goodnews_base, get_dataset
...
for sub_idx in range(0,len(eval_dataset),step):
    ...
    for pred, gt in zip(outputs[0], outputs[1]):
        ...
        #article_len = article_index[valid_count,1]-article_index[valid_count,0]
        #end_ind = start_ind+article_len
        #assert end_ind-start_ind==article_len
        valid_count+=1

        pred_tensor = torch.from_numpy(pred[start_ind-1:end_ind])
        gt_tensor = torch.from_numpy(gt[start_ind:end_ind+1])
        
        if len(pred_tensor)!=len(gt_tensor):
            print(total_count)
            print(pred_tensor.shape, gt_tensor.shape)
            print(start_ind, end_ind, article_len)


        loss = F.cross_entropy(pred_tensor,gt_tensor,reduction='mean').numpy()
        ppl_ex.append(float(loss))
        #wholeloss = F.cross_entropy(torch.from_numpy(pred[:-1]), torch.from_numpy(gt[1:]), reduction='mean').numpy()
        #ppl_ex_whole.append(float(wholeloss))

    #break
ppl_ex = np.array(ppl_ex)
#ppl_ex_whole = np.array(ppl_ex_whole)
print("Article perplexity is {:.3f}".format(math.exp(np.mean(ppl_ex))), flush=True)
#print("Whole perplexity is {:.3f}".format(math.exp(np.mean(ppl_ex_whole))), flush=True)
