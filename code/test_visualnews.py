...
from dataloader import get_visualnews
...
# Get datasets
train_dataset = get_visualnews(data_args, tokenizer=tokenizer) if training_args.do_train else None
eval_dataset = get_visualnews(data_args, tokenizer=tokenizer, evaluate=True) if training_args.do_eval else None
...
ppl_ex = []
# try:
#     article_index = np.load(os.path.join(dir_name,'test_article_beginend_index.npy'))
# except:
#     print("skip loading article length file")
...
for sub_idx in range(0,len(eval_dataset),step):
    ...
    for pred, gt in zip(outputs[0], outputs[1]):
        ...
        end_ind = min(1023, ind_where(gt, target=tokenizer.encode("<|endofarticle|>")[0], default_value=gt.shape[0]))

        if start_ind == 0: # there is no "<|beginarticle|>" token, skip this sample
            special_count+=1
            continue

        pred_tensor = torch.from_numpy(pred[start_ind-1:end_ind])
    ...
    #    ppl_ex.append(logprobs_i[start_ind-1:end_ind])
